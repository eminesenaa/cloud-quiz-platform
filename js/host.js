...
function copyPin() {
  ...
  if (pin && pin !== "------") {
    navigator.clipboard
      .writeText(pin)
      .then(() => {
        toast("PIN copied successfully");
      })
      .catch((err) => {
        toast("Failed to copy PIN");
        console.error(err);
      });
  }
}
...
function syncHostUI(d) {
  ...
  document.getElementById("hchips").innerHTML = ps
    .map((n) => `<span class="chip">${n}</span>`)
    .join("");
  if (d.status === "question") {
    const cnt = Object.values(d.answers || {}).filter(
      (a) => a.qIdx === d.currentQ,
    ).length;
...
async function hostBegin() {
  if (!fbOK) {
    ...
    if (!Object.keys(d.players || {}).length)
      return toast("Demo mode: Join from another tab first");
  }
  window.qIdx = 0;
  await showHQ(0);
}
...
function bcH(msg) {
  if (window._hbc) window._hbc.postMessage(msg);
}
window.bcH = bcH;
...
async function hostSaveQuiz() {
  if (!curUser) {
    toast("Sign in to save quizzes");
    return openAuth();
  }
  if (!quiz) return toast("No quiz available");
  ld(true);
  try {
