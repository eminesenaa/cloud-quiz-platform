...
let punsub = null;
let myNick = '', myScore = 0, myUid = '', myAns = null;
  ...
  try {
    if (fbOK) {
      const s = await db.collection('sessions')
        ...
        .limit(1).get();
      if (s.empty) { ld(false); return toast('❌ Invalid PIN or game already started'); }
      const doc = s.docs[0];
      window.session = { pin, id: doc.id };
      await db.collection('sessions').doc(doc.id).update({ [`players.${myUid}`]: nick });
      startPL();
    } else {
      ...
      window._pbc.onmessage = e => { if (e.data.type === 'upd') handlePUpd(e.data.data); };
      clearInterval(window._poll);
      let _last = '';
      window._poll = setInterval(() => {
        const raw = localStorage.getItem(LS) || '{}';
        if (raw !== _last) { _last = raw; handlePUpd(JSON.parse(raw)); }
      }, 400);
    }
...
function startPL() {
  if (punsub) punsub();
  punsub = db.collection('sessions').doc(session.id).onSnapshot(s => {
    if (!s.exists) return;
    handlePUpd(s.data());
  });
}
...
function renderPQ(d) {
  ...
  document.getElementById('popts').innerHTML = d.questionOptions.map((o, i) => `
...
async function submitAns(opt, qIdx, totalTime) {
  ...
  if (fbOK) {
    await db.collection('sessions').doc(session.id).update({ [`answers.${myUid}`]: ans });
  } else if (window._pbc) {
...
function renderPR(d) {
  ...
  showPV('pv-r');
  document.getElementById('premi').textContent = ok ? '✅' : '❌';
  document.getElementById('prlbl').textContent = ok ? 'Correct!' : 'Wrong Answer';
...
function renderPF(d) {
  ...
  clearInterval(window._poll);
  if (punsub) punsub();
  if (window._pbc) { window._pbc.close(); window._pbc = null; }

  myScore = (d.scores || {})[myUid] || 0;
  const sorted = Object.values(d.scores || {}).sort((a, b) => b - a);
  document.getElementById('pfscore').textContent = myScore;
  document.getElementById('pfrank').textContent = `You finished #${sorted.indexOf(myScore) + 1} 🎉`;
  showPV('pv-f');
}